...
export function VoteButton({ count, onVote, disabled, size = "md" }: Props) {
  ...
  const sizeClasses = { sm: "px-2 py-1 text-xs gap-1", md: "px-3 py-2 text-sm gap-1.5", lg: "px-4 py-3 text-base gap-2" };
  const iconSize = { sm: "w-3 h-3", md: "w-4 h-4", lg: "w-5 h-5" };

  return (
    <button onClick={handleVote} disabled={disabled || isVoting || hasVoted}
      className={cn(
        "flex items-center font-black uppercase tracking-wider border-2 transition-all duration-200 active:scale-90",
        sizeClasses[size],
        hasVoted ? "bg-primary text-primary-foreground border-primary"
      ...
      <span className={cn("transition-all duration-300", animate && "scale-125")}>{localCount}</span>
